...
void airmap::glib::Airspaces::search(const Search::Parameters& parameters, const Search::Callback& cb) {
...
void airmap::glib::Airspaces::for_ids(const ForIds::Parameters& parameters, const ForIds::Callback& cb) {
  std::unordered_map<std::string, std::string> query, headers;
  api_.get("https://api.airmap.com", (boost::format("/airspace/v2/%1%") % parameters.id).str(), std::move(query),
           std::move(headers), [cb](const Api::DoResult& result) {
             if (result) {
               ...
               if (j[jsend::key::status] == jsend::status::success) {
                 ...
                 cb(ForIds::Result(std::make_exception_ptr(std::runtime_error(jsend::stringify_error(j)))));
               }
             }
           });
}
