...
void airmap::rest::Flights::create_flight_by_point(const CreateFlight::Parameters& parameters,
  ...
  requester_->post("/point", std::move(headers), j.dump(), [cb](const net::http::Requester::Result& result) {
    if (result) {
      cb(jsend::to_outcome<Flight>(json::parse(result.value().body)));
    } else {
...
void airmap::rest::Flights::create_flight_by_path(const CreateFlight::Parameters& parameters,
  ...
  requester_->post("/path", std::move(headers), j.dump(), [cb](const net::http::Requester::Result& result) {
    if (result) {
      cb(jsend::to_outcome<Flight>(json::parse(result.value().body)));
    } else {
      cb(CreateFlight::Result{result.error()});
...
void airmap::rest::Flights::create_flight_by_polygon(const CreateFlight::Parameters& parameters,
  ...
  requester_->post("/polygon", std::move(headers), j.dump(), [cb](const net::http::Requester::Result& result) {
    if (result) {
      cb(jsend::to_outcome<Flight>(json::parse(result.value().body)));
    } else {
      cb(CreateFlight::Result{result.error()});
