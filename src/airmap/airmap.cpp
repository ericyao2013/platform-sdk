...
constexpr const char* api_key{
    ...
    "eyJjcmVkZW50aWFsX2lkIjoiY3JlZGVudGlhbHxrOEFkMmxtU29PWXlHR1NlcWdMMGJpTzNvbW1tIiwiYXBwbGljYXRpb25faWQiOiJhcHBsaWNhdG"
  ...
  airmap::Client::create_with_credentials(
      airmap::Client::Credentials{api_key}, ss, di, [&client](const airmap::Client::CreateResult& result) {
            ...
            [client](const airmap::Authenticator::AuthenticateAnonymously::Result& result) {
              ...
              if (result) {
                ...
                client->flights().create_flight_by_polygon(
                    params, [ auth = result.value().id, client ](const airmap::Flights::CreateFlight::Result& result) {
                ...
                /*client->flights().create_flight_by_polygon(
                    params, [ auth = result.value().id, client ](const airmap::Flights::CreateFlight::Result& result) {
                      ...
                      if (result) {
                        ...
                        client->flights().end_flight(airmap::Flights::EndFlight::Parameters{auth, result.value().id},
                                                     [](const airmap::Flights::EndFlight::Result& result) {
                                                       std::cout
                                                           << "Finished ending flight "
                                                           << (result.has_error() ? "with error." : "successfully.")
                                                           << std::endl;
                                                     });
                      }
                    });*/
        ...
        /*client->aircrafts().models(
            airmap::Aircrafts::Models::Parameters{}, [client](const airmap::Aircrafts::Models::Result& result) {
              std::cout << "Finished querying for aircraft models "
                        << (result.has_error() ? "with error." : "successfully.") << std::endl;
              if (result.has_value()) {
                for (const auto& aircraft : result.value()) {
                  std::cout << "  " << aircraft.id << " " << aircraft.name << " " << aircraft.manufacturer.id << " "
                            << aircraft.manufacturer.name << std::endl;
                  client->aircrafts().model_for_id(airmap::Aircrafts::ModelForId::Parameters{aircraft.id},
                                                   [client](const airmap::Aircrafts::ModelForId::Result& result) {
                                                     std::cout << "Finished querying for aircraft model "
                                                               << (result.has_error() ? "with error." : "successfully.")
                                                               << std::endl;
                                                     if (result.has_value()) {
                                                       std::cout << "  " << result.value().id << " "
                                                                 ...
                                                                 << result.value().manufacturer.name << std::endl;
                                                     }
                                                   });
                }
              }
        ...
        client->aircrafts().manufacturers(
            airmap::Aircrafts::Manufacturers::Parameters{}, [](const airmap::Aircrafts::Manufacturers::Result& result) {
              std::cout << "Finished querying for aircraft manufacturers "
                        << (result.has_error() ? "with error." : "successfully.") << std::endl;
              if (result.has_value()) {
        ...
        client->airspaces().search(params, [client](const airmap::Airspaces::Search::Result& result) {
          std::cout << "Finished querying for airspaces " << (result.has_error() ? "with error." : "successfully.")
                    << std::endl;
          if (result.has_value()) {
            for (const auto& airspace : result.value()) {
              ...
              client->airspaces().for_ids({airspace.id()}, [client](const airmap::Airspaces::ForIds::Result& result) {
                std::cout << "Finished querying airspaces for ids "
                          << (result.has_error() ? "with error." : "successfully.") << std::endl;
                if (result.has_value())
                  for (const auto& airspace : result.value())
                    std::cout << "  " << airspace.id() << " " << airspace.name() << " " << airspace.city() << " "
        ...
        client->flights().search(fsp, [client](const airmap::Flights::Search::Result& result) {
          std::cout << "Finished flights query " << (result.has_error() ? "with error." : "successfully.") << std::endl;
          if (result) {
            for (const auto& flight : result.value()) {
              std::cout << "  " << flight.id << " " << flight.pilot.id << " " << flight.aircraft.id << " "
                        << flight.created_at << " " << flight.start_time << " " << flight.end_time << std::endl;
              airmap::Flights::ForId::Parameters fip;
              fip.id = flight.id;
              fip.enhance = true;

              client->flights().for_id(fip, [client](const airmap::Flights::ForId::Result& result) {
                std::cout << "Finished flight query " << (result.has_error() ? "with error." : "successfully.")
                          << std::endl;
                if (result) {
        ...
        std::endl;
            ...
            if (result) {
                client->flights().start_flight_communications(
                            ...
                            [client, flight = result.value()](const airmap::Flights::StartFlightCommunications::Result&
        result) {
                    if (result) {
