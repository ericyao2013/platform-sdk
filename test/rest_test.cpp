...
struct MockCommunicator : public airmap::rest::Communicator {
  MAKE_MOCK5(get,
             void(const std::string&, const std::string&, StringMap&&, StringMap&&, DoCallback),
             override);
  MAKE_MOCK5(post,
             void(const std::string&, const std::string&, StringMap&&, const std::string&,
                  DoCallback),
             override);
...
TEST_CASE("rest") {
  ...
  SECTION("aircrafts") {
    ...
    SECTION("search for models issues get request with correct parameters") {
      ...
      REQUIRE_CALL(communicator,
                   get(mock::eq<std::string>("https://api.airmap.com"),
                       mock::eq<std::string>("/aircraft/v2/model"), mock::eq<StringMap>(query),
                       ANY(StringMap), ANY(airmap::rest::Communicator::DoCallback)));
  ...
  SECTION("airspaces") {
    SECTION("search issues get request with correct parameters") {
      ...
      REQUIRE_CALL(communicator,
                   get(mock::eq<std::string>("https://api.airmap.com"),
                       mock::eq<std::string>("/airspace/v2/search"), mock::eq<StringMap>(query),
                       ANY(StringMap), ANY(airmap::rest::Communicator::DoCallback)));
