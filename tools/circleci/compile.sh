...
docker build \
    ...
    --build-arg AIRMAP_CIRCLECI_PASSWORD=${test_pilot_099_password} --force-rm=true -t airmapd.alpine:latest -f alpine docker 
...
docker build \
    ...
    --build-arg AIRMAP_CIRCLECI_PASSWORD=${test_pilot_098_password} --force-rm=true -t airmapd.ubuntu:latest -f ubuntu docker 
